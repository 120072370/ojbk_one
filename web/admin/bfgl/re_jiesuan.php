...
$sql		=	"select k_bet.*,k_user.username from k_bet left join k_user on k_bet.uid=k_user.uid where k_bet.lose_ok=1 and k_bet.status not in(0,6,7) and match_id in($mid) order by  k_bet.bid desc"; //单式
$result		=	$mysqli->query($sql);

$sql		=	"select k_bet_cg.*,k_user.username,k_bet_cg_group.win,k_bet_cg_group.www from k_bet_cg left join k_user on k_bet_cg.uid=k_user.uid left join k_bet_cg_group on k_bet_cg_group.gid=k_bet_cg.gid where match_id in($mid) and k_bet_cg.status not in(0) order by k_bet_cg.bid desc";
