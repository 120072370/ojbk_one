...
?>
        ...
        <td width="200"><a href="zqbf.php?date=<?=$page_date?>" style="font-size:13px;">&lt;&lt;返回未结算</a>
		...
		【<a href="?date=<?=$page_date?>&type=1" style="font-size:13px;">只看下注</a>】&nbsp;&nbsp;
          【<a href="?date=<?=$page_date?>&type=0" style="font-size:13px;">查看所有</a>】</td>
  ...
  <table   border="0" cellspacing="1" cellpadding="0"  bgcolor="006255" width="900" height="41">
	...
	if($_GET["type"]!="0")
	{
		...
		$sqlwhere.=" AND ((Match_ID in (select match_id from dy1_db.k_bet where match_coverdate>='$begin_time' and match_coverdate<='$end_time'))";
		$sqlwhere.=" OR (Match_ID in (select match_id from dy1_db.k_bet_cg where match_endtime>='$begin_time')))";
	}
	
   $sql			=	"SELECT Match_ID,Match_Time,Match_Master,Match_Guest,Match_Name,MB_Inball,TG_Inball,MB_Inball_HR,TG_Inball_HR FROM Bet_Match where match_js=1 and (match_Date='$page_date' or match_date='$page_date2') ".$sqlwhere." order by Match_CoverDate,match_name,Match_Master desc";
