...
<?php
		...
		$uid	=	0; //会员id
		$sql	=	"select uid from k_user where username='$un' limit 0,1"; //取出uid
		$query	=	$mysqli->query($sql);
		if($row = $query->fetch_array()){
			$uid	=	$row['uid'];
		}
		
		$sql	=	"select jkje,jyzs,flbl,flje,id from k_hyfl where uid=$uid and month='$yf' limit 0,1"; //检查该用户是否已返利
		echo $sql;
		$query	=	$mysqli->query($sql);
		if($row = $query->fetch_array()){
?>
