...
<div id="pageMain">
	...
	$cj=$_GET['top'];
		...
		switch($cj){
			...
			case 2:
				...
				if ($_GET['agents']!=''){
					$sqlwhere=" and agents='{$_GET['agents']}'";}
				else{
					$sqlwhere=" and (agents is null or agents='')";}
				$display="会员";
				break;
			}
	?>
	...
	<table width="100%" border="0" cellpadding="5" cellspacing="1" class="font12" style="margin-top:5px;line-height:20px;" bgcolor="#798EB9">
		...
		//六合彩
		if(0&&in_array("lhc",$cz)){
			
			if($sql_cz!=""){
				$sql_cz	.=	" union all ";
			}
			$sql_cz	.=	"select $whats if(js=1,1,0) as y_num,if(js=1,sum_m,0) as y_bet_money,if(js=1 and bm<>2,sum_m,0) as yx_bet_money,if(js=1,(case bm when 1 then sum_m*rate when 2 then sum_m else 0 end),0) as y_win,if(js=1,(case bm when 2 then 0 else sum_m*user_ds/100 end),0) as y_fs,if(js=0,1,0) as w_num,if(js=0,sum_m,0) as w_bet_money from dy1_db.c_bet k left outer join dy1_db.k_user u on k.username=u.username where k.adddate>='$cn_q_btime' and k.adddate<='$cn_q_etime' ".$sqlwhere;
		}
		//重庆时时彩
		...
		<tr align="center" onMouseOver="this.style.backgroundColor='<?=$over?>'" onMouseOut="this.style.backgroundColor='<?=$out?>'" style="background-color:<?=$color?>;">
			...
			case 0:
				if ($row["y_num"]==''){
					$html="<td><a href='?top=1&$links'>admin(站主)</a></td><td>0</td>";}
				else{
					$html="<td><a href='?top=1&$links'>admin(站主)</a></td><td>".$row["y_num"]."</td>";}
				break;
			case 1:
				if ($row["agents"]==''){
				$html="<td><a href='?top=2&$links&agents=' style='color:red'>none</a></td><td>".$row["y_num"]."</td>";}
				else{
				$html="<td><a href='?top=2&$links&agents=".$row["agents"]."'>".$row["agents"]."</a></td><td>".$row["y_num"]."</td>";}
				break;
			case 2:
		...
		<tr align="center" onMouseOver="this.style.backgroundColor='<?=$over?>'" onMouseOut="this.style.backgroundColor='<?=$out?>'" style="background-color:<?=$color?>;">
			<?php
				$sql ="SELECT COUNT(*) total ,SUM(BetAmount)  betAmount ,SUM(Commissionable)  validBetAmount ,SUM(netAmount)  netAmount ,SUM(reamount)  reamount  FROM   api_bbbetdetail where WagersDate>='$q_btime' AND WagersDate<='$q_etime'";
				$query		=	$mysqli->query($sql);
				$rs			=	$query->fetch_array();
				$agtotal	=	$rs['total'];
				$betAmount	=	$rs['betAmount'];
				$validBetAmount	=	$rs['validBetAmount'];
				$netAmount	=	$rs['netAmount'];	
				$reamount	=	$rs['reamount'];		
				$y_yingkui	=	sprintf("%.2f",$netAmount+$reamount);
				$sum_y_pc = sprintf("%.2f",$netAmount+$betAmount);

			$sum_y_num			+=	$agtotal;
			$sum_y_bet_money	+=	$betAmount;
			$sum_yx_bet_money	+=	$validBetAmount;
			$sum_y_win			+=	$sum_y_pc;
			$sum_y_fs			+=	$reamount;
			$sum_y_yingkui		+=	$y_yingkui;	
			?>
			<td>admin(BBIN娱乐)</a><td><?=$agtotal?></td>
			<td><?=sprintf("%.2f",$betAmount)?></td>
			<td><?=sprintf("%.2f",$validBetAmount)?></td>
			<td><?=sprintf("%.2f",$sum_y_pc)?></td>
			<td><?=sprintf("%.2f",$reamount)?></td>
			<td style="color:<?=$y_yingkui>=0?'#ff0000':'#009900'?>"><?=sprintf("%.2f",$y_yingkui)?></td>
			<td>0</td>
			<td>0.00</td>
			<td><?=$agtotal?></td>
			<td><?=sprintf("%.2f",$betAmount)?></td>
