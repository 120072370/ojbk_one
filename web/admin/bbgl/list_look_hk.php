  ...
  <tr>
    <td height="24" nowrap bgcolor="#FFFFFF"><table width="100%" border="1" bgcolor="#FFFFFF" bordercolor="#96B697" cellspacing="0" cellpadding="0" style="border-collapse: collapse; color: #225d9c;" >
      <tr bgcolor="efe" class="t-title" align="center">
        <td width="7%" height="24" ><strong>编号</strong></td>
        ...
        <td width="10%"><strong>汇款金额</strong></td>
        <td width="10%"><strong>赠送金额</strong></td>
        <td width="10%" ><strong>汇款银行</strong></td>
        <td width="29%" ><strong>汇款方式/汇款地址</strong></td>
...
$money		=	$zsje	=	0;
$sql		=	"select * from huikuan where `status`=1 and uid=$uid and `adddate`>='$stime1' and `adddate`<='$etime1' order by id desc";
...
?> 
      <tr align="center" onMouseOver="this.style.backgroundColor='#EBEBEB'" onMouseOut="this.style.backgroundColor='#ffffff'" >
        ...
        <td><?=$rows["lsh"]?><br/><?=$time=='EN' ? $rows["adddate"] : get_bj_time($rows["adddate"])?></td>
        <td><?=$rows["money"]?></td>
        <td><?=$rows["zsjr"]?></td>
        <td><?=$rows["bank"]?></td>
        <td><?=$rows["manner"]?><br/><?=$rows["address"]?></td>
        <td><span style="color:#006600;">汇款成功</span></td>
        </tr>  	
<?php
}
?>
    ...
    <tr>
      <td >该页统计：总汇款：<span style="color:#FF0000"><?=$money?></span>，赠送金额：<span style="color:#FF00FF"><?=$zsje?></span></td>
     </tr>
</table>
</body>
</html>