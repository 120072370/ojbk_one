  ...
  <tr>
    <td height="24" nowrap bgcolor="#FFFFFF"><table width="100%" border="1" bgcolor="#FFFFFF" bordercolor="#96B697" cellspacing="0" cellpadding="0" style="border-collapse: collapse; color: #225d9c;" >
      <tr bgcolor="efe" class="t-title" align="center">
        ...
        <td width="17%" ><strong>订单号/申请时间</strong></td>
        <td width="7%"><strong>提款金额</strong></td>
        <td width="7%"><strong>手续费</strong></td>
        <td width="17%" ><strong>开户行/卡号</strong></td>
        <td width="31%" ><strong>开户人/开户地址</strong></td>
...
$money		=	$sxf	=	0;
$sql		=	"select * from k_money where m_value<0 and `status`=1 and uid=$uid and m_make_time>='$stime1' and m_make_time<='$etime1' order by m_id desc";
...
?> 
      <tr align="center" onMouseOver="this.style.backgroundColor='#EBEBEB'" onMouseOut="this.style.backgroundColor='#ffffff'">
        ...
        <td><?=$rows["m_order"]?><br/><?=$time=='EN' ? $rows["m_make_time"] : get_bj_time($rows["m_make_time"])?></td>
        <td><?=abs($rows["m_value"])?></td>
        <td><?=$rows["sxf"]?></td>
        <td><?=$rows["pay_card"]?><br/><?=$rows["pay_num"]?></td>
        <td><?=$rows["pay_name"]?><br/><?=$rows["pay_address"]?></td>
        <td><span style="color:#006600;">取款成功</span></td>
        </tr>    	
<?php
}
?>
    ...
    <tr>
      <td >该页统计：总取款：<span style="color:#FF0000"><?=$money?></span>，手续费：<span style="color:#FF00FF"><?=$sxf?></span></td>
     </tr>
</table>
</body>
</html>