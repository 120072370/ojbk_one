...
if($_GET["action"]=="save"){
	...
	$sql			=	"update t_guanjun set match_name='$match_name',x_title='$x_title',match_date='$match_date',match_time='$match_time',match_coverdate='$match_coverdate',match_id=$match_id where x_id=".$_GET["id"];
	$mysqlis->query($sql);
    if($mysqlis->affected_rows == 1){
		include_once("../../class/admin.php");
    	admin::insert_log($_SESSION["adminid"],"修改了冠军项目 $x_title");
		
		message('修改成功','list.php?type=1');
    }else{
		message('修改失败','list.php?type=1');
    }	
}
$sql	=	"select * from t_guanjun where x_id=".$_GET["id"];
...
<table width="100%" border="0" cellpadding="3" cellspacing="1" bgcolor="#CCCCCC">
  <tr>
    <td height="24" nowrap background="../images/06.gif"><font >&nbsp;<span class="STYLE2">冠军管理：编辑冠军项目</span></font></td>
  </tr>
  <tr>
    <td height="24" align="center" nowrap bgcolor="#FFFFFF">
<br>
<form action="edit.php?action=save&id=<?=$_GET["id"]?>" method="post" name="form1" onSubmit="return check();">
<table width="90%" align="center" border="1" bgcolor="#FFFFFF" bordercolor="#96B697" cellspacing="0" cellpadding="0" style="border-collapse: collapse; color: #225d9c;">
  <tr>
    ...
    <td><input type="text" class="inputguanjun"  name="x_title" id="x_title" value="<?=$rows["x_title"]?>" /></td>
  </tr>
  <tr>
    <td bgcolor="#F0FFFF">项目名称</td>
    <td><input type="text" class="inputguanjun"  name="match_name" id="match_name" value="<?=$rows["match_name"]?>" /></td>
  </tr>
  <tr>
    <td width="172" bgcolor="#F0FFFF">正网编号</td>
    <td width="473"><input type="text"  name="match_id" id="match_id" value="<?=$rows["match_id"]?>" />
      格式为：2217</td>
  </tr>
  <tr>
    <td width="172" bgcolor="#F0FFFF">结束日期</td>
    <td width="473"><input type="text"  name="match_date" id="match_date" value="<?=$rows["match_date"]?>" />
      格式为：08-30</td>
  </tr>
  <tr>
    <td bgcolor="#F0FFFF">结束时间</td>
    <td><input type="text"  name="match_time" id="match_time" value="<?=$rows["match_time"]?>" /> 
      格式为：11:00a</td>
  </tr>
    <tr>
