...
if($_REQUEST['ac']=='re'){
	...
	$sql		= "select * from c_auto_15 where qishu=".$qi." order by id desc limit 1";
}else{
	$sql		= "select * from c_auto_15 where ok=0";
}
...
$hm[]		= $rs['ball_5'];
//根据期数读取未结算的注单
$sql1		= "select * from c_bet where type='分分彩' and js=0 and qishu=".$qi." order by addtime asc";
...
$msql="update c_auto_15 set ok=1 where qishu=".$qi."";
$mysqli->query($msql) or die ("期数修改失败!!!");
$ii++;
}
if($i<=0){
	$sql2		= "select c_bet.*,auto.ball_1,auto.ball_2,auto.ball_3,auto.ball_4,auto.ball_5 from c_bet left join c_auto_15 AS auto ON auto.qishu=c_bet.qishu where c_bet.type='分分彩' and c_bet.js=0 order by c_bet.addtime asc";
	$query2		= $mysqli->query($sql2);
	while($rows2 = $query2->fetch_array()){
		if($rows2['ball_1']<>NULL && $rows2['ball_2']<>NULL && $rows2['ball_3']<>NULL && $rows2['ball_4']<>NULL && $rows2['ball_5']<>NULL){
			//print_r($rows2);echo "<br>";
			if(jss($rows2)=='OK'){
				echo $rows2['id'].":漏单结算成功<br>";
			}
		}
	}
}
...
function jss($rows){
	...
	//开始结算第一球
	if($rows['mingxi_1']=='第一球'){
		$ds = Ssc_Ds($rows['ball_1']);
		$dx = Ssc_Dx($rows['ball_1']);
		if($rows['mingxi_2']==$rows['ball_1'] || $rows['mingxi_2']==$ds || $rows['mingxi_2']==$dx){
			//如果投注内容等于第一球开奖号码，则视为中奖
			$msql="update c_bet set js=1 where id='".$rows['id']."'";
			$mysqli->query($msql) or die ("注单修改失败!!!".$rows['id']);
			//注单中奖，给会员账户增加奖金
...
$mysqli->query($msql2);
	...
	//开始结算第二球
	if($rows['mingxi_1']=='第二球'){
		$ds = Ssc_Ds($rows['ball_2']);
		$dx = Ssc_Dx($rows['ball_2']);
		if($rows['mingxi_2']==$rows['ball_2'] || $rows['mingxi_2']==$ds || $rows['mingxi_2']==$dx){
			//如果投注内容等于第一球开奖号码，则视为中奖
			$msql="update c_bet set js=1 where id='".$rows['id']."'";
			$mysqli->query($msql) or die ("注单修改失败!!!".$rows['id']);
			//注单中奖，给会员账户增加奖金
...
$mysqli->query($msql2);
	...
	//开始结算第三球
	if($rows['mingxi_1']=='第三球'){
		$ds = Ssc_Ds($rows['ball_3']);
		$dx = Ssc_Dx($rows['ball_3']);
		if($rows['mingxi_2']==$rows['ball_3'] || $rows['mingxi_2']==$ds || $rows['mingxi_2']==$dx){
			//如果投注内容等于第一球开奖号码，则视为中奖
			$msql="update c_bet set js=1 where id='".$rows['id']."'";
			$mysqli->query($msql) or die ("注单修改失败!!!".$rows['id']);
			//注单中奖，给会员账户增加奖金
...
$mysqli->query($msql2);
	...
	//开始结算第四球
	if($rows['mingxi_1']=='第四球'){
		$ds = Ssc_Ds($rows['ball_4']);
		$dx = Ssc_Dx($rows['ball_4']);
		if($rows['mingxi_2']==$rows['ball_4'] || $rows['mingxi_2']==$ds || $rows['mingxi_2']==$dx){
			//如果投注内容等于第一球开奖号码，则视为中奖
			$msql="update c_bet set js=1 where id='".$rows['id']."'";
			$mysqli->query($msql) or die ("注单修改失败!!!".$rows['id']);
			//注单中奖，给会员账户增加奖金
...
$mysqli->query($msql2);
	...
	//开始结算第五球
	if($rows['mingxi_1']=='第五球'){
		$ds = Ssc_Ds($rows['ball_5']);
		$dx = Ssc_Dx($rows['ball_5']);
		if($rows['mingxi_2']==$rows['ball_5'] || $rows['mingxi_2']==$ds || $rows['mingxi_2']==$dx){
			//如果投注内容等于第一球开奖号码，则视为中奖
			$msql="update c_bet set js=1 where id='".$rows['id']."'";
			$mysqli->query($msql) or die ("注单修改失败!!!".$rows['id']);
			//注单中奖，给会员账户增加奖金
...
$mysqli->query($msql2);
    ...
    //
        //填写开奖结果到注单
    $msql="update c_bet set jieguo='".$rows['ball_1'].",".$rows['ball_2'].",".$rows['ball_3'].",".$rows['ball_4'].",".$rows['ball_5']."' where id=".$rows['id']."";
    $mysqli->query($msql) or die ("会员修改失败!!!".$rows['id']);
	//==============返水开始============
	$sql_f	=	"select cpfsbl from k_group left join k_user ON k_group.id=k_user.gid where k_user.uid='".$rows['uid']."' limit 1";
	$query_f	=	$mysqli->query($sql_f);
