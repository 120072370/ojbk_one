...
$hm[]		= $rs['ball_3'];
//根据期数读取未结算的注单
$sql1		= "select * from c_bet where type='福彩3D' and js=0 and qishu=".$qi." order by addtime asc";
...
$mysqli->query($msql2);
	//填写开奖结果到注单
    $msql="update c_bet set jieguo='".implode(',',$hm)."' where id=".$rows['id']."";
    $mysqli->query($msql) or die ("写入结果失败!!!".$rows['id']);
	//==============返水开始============
