...
$curl->set_referrer("http://old.1680210.com/MarkSix");
...
$html_data = $curl->fetch_url("http://old.1680210.com/Open/CurrentOpenOne?code=4001");
$a=array('(',')');
$b=array('[',']');
$msg='';
...
if($arr['c_t']){
	...
	if($six_set['auto_pan']==1){
		...
		//$opentime=date('Y-m-d H:i:s',time()+1*12*3600);
...
<table width="100%" border="0" cellpadding="0" cellspacing="0">
  <tr> 
    <td align="left">
      ...
      SIX<br>采集到<?=$m?>期(<?=$qishu?>期<?="$ball_1,$ball_2,$ball_3,$ball_4,$ball_5,$ball_6,$ball_7"?>)<?=$msg ? '<br>'.$msg.'<br>' : ''?>
      <span id="timeinfo"></span>
      </td>
  </tr>
</table>
<? if($six_set['auto_jie']==1){?>
<iframe src="./js/js_six.php?qi=<?=$qishu?>" frameborder="0" scrolling="no" height="100" width="100%"></iframe>
<? }?>
</body>
</html>s