...
//for($pages=0;$pages<$t_page;$pages++){
	$data=theif_data($webdb["datesite"],$webdb["cookie"],'BKR','r_main',$langx,$pages);
	...
	if (sizeof(explode("gamount",$data))>1){
		...
		for($i=0;$i<$cou;$i++){
			...
			if($datainfo[0]+0!=0){
				...
				$datainfo[22]    =    substr($datainfo[22],1,strlen($datainfo[22])-1);
				
				$datainfo[26]    =    str_replace(' ','',$datainfo[26]);
				$datainfo[26]    =    substr($datainfo[26],1,strlen($datainfo[26])-1);
				...
				if($mysqlis->affected_rows){ //有数据，更新
					if($match_type==1){
						$sql	=	"update lq_match set Match_Date='$time[0]',Match_Time='$time[1]',Match_IsLose=$isLose,Match_Type=$match_type,Match_ShowType='$datainfo[7]',Match_Ho='$datainfo[9]',Match_Ao='$datainfo[10]',Match_RGG='$datainfo[8]',Match_DxGG='$datainfo[11]',Match_DxDpl='$datainfo[14]',Match_DxXpl='$datainfo[13]',Match_DsDpl='$datainfo[20]',Match_DsSpl='$datainfo[21]',Match_CoverDate='$CoverDate',Match_MasterID='$datainfo[3]',Match_GuestID='$datainfo[4]',
						Match_BzM='$datainfo[15]',
						Match_BzG='$datainfo[16]',
						Match_DFzDX='$datainfo[22]',
						Match_DFzDpl='$datainfo[24]',
						Match_DFzXpl='$datainfo[25]',
						Match_DFkDX='$datainfo[26]',
						Match_DFkDpl='$datainfo[28]',
						Match_DFkXpl='$datainfo[29]',
						Match_LstTime=now(),iPage=".($pages+1).",iSn=".($i+1)." WHERE Match_ID=$datainfo[0]  AND Match_StopUpdateds=0;";
					}else{
						$sql	=	"update lq_match set Match_Date='$time[0]',Match_Time='$time[1]',Match_IsLose=$isLose,Match_Type=$match_type,Match_ShowType='$datainfo[7]',Match_Ho='$datainfo[9]',Match_Ao='$datainfo[10]',Match_RGG='$datainfo[8]',Match_DxGG='$datainfo[11]',Match_DxDpl='$datainfo[14]',Match_DxXpl='$datainfo[13]',Match_DsDpl='$datainfo[20]',Match_DsSpl='$datainfo[21]',Match_CoverDate='$CoverDate',Match_MasterID='$datainfo[3]',Match_GuestID='$datainfo[4]',
						Match_BzM='$datainfo[15]',
						Match_BzG='$datainfo[16]',
						...
						Match_DFkDpl='$datainfo[28]',
						Match_DFkXpl='$datainfo[29]',
						Match_LstTime=now(),iPage=".($pages+1).",iSn=".($i+1)." WHERE Match_ID=$datainfo[0]  AND Match_StopUpdatezc=0;";
					}
				}else{ //没数据，插入
					$addname='Match_BzM,Match_BzG,Match_DFzDX,Match_DFzDpl,Match_DFzXpl,Match_DFkDX,Match_DFkDpl,Match_DFkXpl';
					$addval="'$datainfo[15]','$datainfo[16]','$datainfo[22]','$datainfo[24]','$datainfo[25]','$datainfo[26]','$datainfo[28]','$datainfo[29]'";
					$sql	=	"insert into lq_match(Match_ID,Match_Date,Match_Time,Match_Name,Match_Master,Match_Guest,Match_IsLose,Match_Type,Match_ShowType,Match_Ho,Match_Ao,Match_RGG,Match_DxGG,Match_DxDpl,Match_DxXpl,Match_DsDpl,Match_DsSpl,Match_CoverDate,Match_MatchTime,Match_MasterID,Match_GuestID,$addname,Match_LstTime,iPage,iSn)values('$datainfo[0]','$time[0]','$time[1]','$datainfo[2]','$datainfo[5]','$datainfo[6]','$isLose','$match_type','$datainfo[7]','$datainfo[9]','$datainfo[10]','$datainfo[8]','$datainfo[11]','$datainfo[14]','$datainfo[13]','$datainfo[20]','$datainfo[21]','$CoverDate','$HgDate','$datainfo[3]','$datainfo[4]',$addval,now(),".($pages+1).",".($i+1).")";
				}
				$mysqlis->query($sql);
				$msg++;
			}
