...
class bet{
    static function set($bid,$status,$mb_inball=NULL,$tg_inball=NULL){ //审核当前投注项1，2， 4，5 赢，输，赢一半，输一半
		...
		$sql_u="select k_user.gid from k_user left join k_bet ON k_bet.uid=k_user.uid where k_bet.bid=$bid";
		$query_u	=	$mysqli->query($sql_u);
		$uid_u		=	$query_u->fetch_array();
		$sql_f	=	"select zqfsbl from k_group where id='".$uid_u['gid']."' limit 1";
