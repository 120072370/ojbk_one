...
function check_submit_money(){
	...
	if($("#newmoneypass1").val().length<=0 || $("#newmoneypass2").val().length<=0 || $("#newmoneypass3").val().length<=0
		|| $("#newmoneypass4").val().length<=0 || $("#newmoneypass5").val().length<=0 || $("#newmoneypass6").val().length<=0){
		$("#newmoneypass_txt").html('请选择6位新取款密码');
		return false;
	}
}
//设置财务资料
