...
/*$sql		=	"select username from k_user where username='".$jsr."' limit 1";
...
try{
	...
	if($q1 == 1 && $q2 == 1){
		    $mysqli->commit(); //事务提交
			if($type=="1"){
				$sql = "insert into k_user_daili (uid,r_name,about,status,add_time) values ($id,'$pay_name','上级代理".$_SESSION['username']."从前台添加',1,now())";
				$mysqli->query($sql);
				echo "<script type='text/javascript'>alert(\"代理注册成功！\");history.back(0);</script>";
			}else{
				echo "<script type='text/javascript'>alert(\"会员注册成功！\");history.back(0);</script>";
			}
			exit();
	}else{
