...
class bet{
    static function set($bid,$status,$mb_inball=NULL,$tg_inball=NULL){ //审核当前投注项1，2， 4，5 赢，输，赢一半，输一半
    	...
    	switch ($status){
    		case "1": //赢
    			$sql	=	"update k_user,k_bet set k_user.money=k_user.money+k_bet.bet_win+(k_bet.bet_money/100),k_bet.win=k_bet.bet_win,k_bet.status=1 ,k_bet.update_time=now(),k_bet.MB_Inball='$mb_inball',k_bet.TG_Inball='$tg_inball',fs=k_bet.bet_money/100 where k_user.uid=k_bet.uid and k_bet.bid=$bid and k_bet.status=0";
                $msg	=	"审核了编号为".$bid."的注单,设为赢";
				break; 
   			case "2": //输
   				$sql	=	"update k_user,k_bet set k_user.money=k_user.money+(k_bet.bet_money/100),status=2,update_time=now(),k_bet.MB_Inball='$mb_inball',k_bet.TG_Inball='$tg_inball',fs=k_bet.bet_money/100 where k_user.uid=k_bet.uid and k_bet.bid=$bid and k_bet.status=0";
   		    ...
   		    case "3": //无效或取消
   				...
   				break;
		    case "4": //赢一半
   				$sql	=	"update k_user,k_bet set k_user.money=k_user.money+k_bet.bet_money+((k_bet.bet_money/2)*k_bet.bet_point)+(k_bet.bet_money/200),k_bet.win=k_bet.bet_money+((k_bet.bet_money/2)*k_bet.bet_point),k_bet.status=4 ,k_bet.update_time=now(),k_bet.MB_Inball='$mb_inball',k_bet.TG_Inball='$tg_inball',fs=k_bet.bet_money/200 where k_user.uid=k_bet.uid and k_bet.bid=$bid and k_bet.status=0";
   				$msg	=	"审核了编号为".$bid."的注单,设为赢一半";
   				break;
			case "5": //输一半
   				$sql	=	"update k_user,k_bet set k_user.money=k_user.money+(k_bet.bet_money/2)+(k_bet.bet_money/200),k_bet.win=k_bet.bet_money/2,k_bet.status=5,k_bet.update_time=now(),k_bet.MB_Inball='$mb_inball',k_bet.TG_Inball='$tg_inball',fs=k_bet.bet_money/200 where k_user.uid=k_bet.uid and k_bet.bid=$bid and k_bet.status=0";
